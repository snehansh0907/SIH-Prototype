// scratch/test-districts-list.mjs
import fs from 'fs';

const ALL_STATES_DISTRICTS = {
  'Andhra Pradesh': [
...
console.log('Total States and UTs:', Object.keys(ALL_STATES_DISTRICTS).length);
let totalDistricts = 0;
for (const [st, dists] of Object.entries(ALL_STATES_DISTRICTS)) {
  totalDistricts += dists.length;
}
console.log('Total Districts in all States/UTs:', totalDistricts);
console.log('Maharashtra districts:', ALL_STATES_DISTRICTS['Maharashtra'].length);
console.log('Odisha districts:', ALL_STATES_DISTRICTS['Odisha'].length);

const indianStatesContent = fs.readFileSync('./src/data/indianStates.ts', 'utf8');
const indianStatesNames = [...indianStatesContent.matchAll(/name:\s*'([^']+)'/g)].map(m => m[1]);
console.log('States in indianStates.ts:', indianStatesNames.length);

let mismatch = false;
for (const name of indianStatesNames) {
  if (!ALL_STATES_DISTRICTS[name]) {
    console.error('MISSING IN DISTRICT LIST:', name);
    mismatch = true;
  }
}
for (const name of Object.keys(ALL_STATES_DISTRICTS)) {
  if (!indianStatesNames.includes(name)) {
    console.error('EXTRA IN DISTRICT LIST:', name);
    mismatch = true;
  }
}
if (!mismatch) {
  console.log('MATCH CHECK PASSED: All 36 states and UTs match 100%!');
}
