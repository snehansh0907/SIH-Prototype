...
/**
 * Helper to get localized Marathi district name from district string,
 * falling back to district name if not found.
 */
export function getDistrictMr(districtName: string): string {
  ...
  return map[clean] || districtName;
}

/**
 * Helper to get localized Hindi district name from district string,
 * falling back to district name if not found.
 */
export function getDistrictHi(districtName: string): string {
  const clean = (districtName || '').toLowerCase().trim();
  const map: Record<string, string> = {
    'nashik': 'नासिक',
    'niphad': 'नासिक',
...
export function getNearbyRegionsForLocation(
  ...
  const homeRegion: NearbyRegion = {
    ...
    districtHi: getDistrictHi(loc.district),
    districtMr: getDistrictMr(loc.district),
    state: loc.state,
      ...
      for (const item of sorted) {
        if (!addedNames.has(item.name.toLowerCase()) && regions.length < 5) {
          ...
          regions.push({
            ...
            districtHi: getDistrictHi(item.district),
            districtMr: getDistrictMr(item.district),
            state: loc.state,
            latitude: item.lat,
            longitude: item.lng,
